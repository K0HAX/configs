...
*** Hook =make-frame= to detect if we're in a TTY or GUI
    ...
    (defun me/pick-color-theme (frame)
      (select-frame frame)
      (if (window-system frame)
          (progn
            (disable-theme 'wheatgrass) ; in case it was active
            (enable-theme 'solarized))
        (progn
          (disable-theme 'solarized)
          (enable-theme 'wheatgrass))))

    (add-hook 'after-make-frame-functions 'me/pick-color-theme)
...
** Optimize org-babel config for faster startup
   ...
   (defun my/delete-config-el ()
       ...
       (defvar my-configel)
       (setq my-configel "~/git/dotfiles/configs/emacs/config.el")
       (if (s-suffix? "emacs/config.org" buffer-file-name)
           (if (file-exists-p my-configel)
               (progn (delete-file "~/git/dotfiles/configs/emacs/config.el")
                      (delete-file "~/git/dotfiles/configs/emacs/config.elc")))))
...
** =git-load=
   ...
   (byte-recompile-directory (expand-file-name "~/.emacs.d/git/") 0)
   (load "git-load")
   #+END_SRC
** ERC IRC Client
...
** Load secrets
   ...
   (eval-after-load "git-load"
     (git-load "~/ownCloud/Dropbox/git/menglehorn-emacs-secret.git" "menglehorn-emacs-secret"))
   #+END_SRC
